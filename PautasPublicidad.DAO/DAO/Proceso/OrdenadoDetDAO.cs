...
{
    public class OrdenadoDetDAO : DAOBase<OrdenadoDetDTO>
    {
        //Methods...
        public void MoverCeros(string pautaId, SqlTransaction tran)
        {
            DAOHelper.EjecutarNonQuery(
                string.Format(
                "UPDATE [OrdenadoDet] SET Costo=0, CostoOP=0, CostoUni=0, CostoOPUni=0 WHERE PautaId = '{0}' AND DatareaId = {1}",
                pautaId, DAOHelper.DatareaId), tran);
        }

        public int GetLastRecId()
        {
            object aux = DAOHelper.EjecutarScalar("SELECT MAX(RECID) FROM ORDENADODET");
            if (aux is DBNull)
                return 0;
            else
